...
// helpers
export type BlockInitializer<Block extends BlockBase<any, any>> = (type: Block["type"]) => Block
...
// --- implements ---
// helper
const findTargetBlockIndex = <Block extends BlockBase<any, any>>(id: Block["id"], blocks: Block[]): number => {
...
export const useBlockCollection = <Block extends BlockBase<any, any>>({
  ...
  const handleSubmit: HandleBlockSubmit<Block> = useCallback((func) => {
    return () => {
      func(blocks)
    }
  }, [])
